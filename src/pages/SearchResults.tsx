...
const SearchResults = () => {
  ...
  const FilterSection = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className={`${isMobile ? 'bg-background border border-border rounded-lg' : 'card-jetleg'} p-6 ${!isMobile ? 'sticky top-6' : ''}`} style={!isMobile ? { backgroundColor: '#bad5ff' } : {}}>
      {isMobile && (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              onChange={e => handleFilterChange('minPassengers', parseInt(e.target.value) || 1)}
              className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:ring-2 focus:ring-accent/20"
              placeholder="1"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={e => handleFilterChange('maxPassengers', parseInt(e.target.value) || 20)}
              className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:ring-2 focus:ring-accent/20"
              placeholder="20"
            />
          </div>
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {isFilterOpen && (
          <div className="fixed inset-0 bg-black/50 z-50 lg:hidden">
            <div className="absolute inset-x-0 top-0 bg-background max-h-screen overflow-y-auto">
